...
The additive-set successor is accepted under
[#127](https://github.com/packetlss/compliance/issues/127) but is not implemented.
This document distinguishes the current experimental representation from that
documentation-only successor contract.
...
## Accepted additive-set successor
...
explicitly opts into additive-set composition, conceptually:
...
A contribution is ordinary applicable policy, conceptually:
...
This is illustrative architecture, not accepted wire syntax. An authored
...
governed current policy. The successor schema/runtime contract must therefore allow:
...
The later implementation must collect all applicable declarations, bases and
...
resource/artifact/digest/identity families are outside #127. Unsupported syntax is
