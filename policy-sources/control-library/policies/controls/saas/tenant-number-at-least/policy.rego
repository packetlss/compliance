package compliance.controls.saas.tenant_number_at_least
...
configuration := [doc |
	...
	doc.type == "saas.tenant.configuration/v1"
]
...
outcome := {
	"status": "unknown",
	"reason": "No fresh saas.tenant.configuration/v1 evidence is available",
	...
	"status": "unknown",
	"reason": sprintf("The SaaS collector could not determine %s.%s", [section, setting]),
	"expected": {section: {setting: {"minimum": minimum}}},
	...
	"status": "fail",
	"reason": sprintf("SaaS setting %s.%s is %v; minimum is %v", [section, setting, observed, minimum]),
	"expected": {section: {setting: {"minimum": minimum}}},
	"observed": {section: {setting: observed}},
} if {
	observed < minimum
	...
	"status": "pass",
	"reason": sprintf("SaaS setting %s.%s is %v, meeting minimum %v", [section, setting, observed, minimum]),
	"expected": {section: {setting: {"minimum": minimum}}},
	"observed": {section: {setting: observed}},
}
